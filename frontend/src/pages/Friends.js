...
function Friends() {
    ...
    const [searchInput, setSearchInput] = useState(''); // 입력창 값 (타이핑 중)
    const [searchTerm, setSearchTerm] = useState(''); // 실제 검색에 적용된 값
    ...
    const loadQuestions = async () => {
        try {
            ...
            const response = await api.get(`/questions/with-status?limit=${PAGE_SIZE}&offset=0&sort=${sortMode}${searchQuery}`);
    ...
    const loadMoreQuestions = async () => {
        ...
        try {
            setLoadingMore(true);
            const searchQuery = searchTerm ? `&search=${encodeURIComponent(searchTerm)}` : '';
            const response = await api.get(`/questions/with-status?limit=${PAGE_SIZE}&offset=${questions.length}&sort=${sortMode}${searchQuery}`);
